...
buildProject:
	rm -rf releases
	mkdir releases
	gox -output="releases/{{.Dir}}_{{.OS}}_{{.Arch}}/{{.Dir}}" -osarch="linux/amd64 linux/arm windows/amd64 windows/386 darwin/amd64" -ldflags="-X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagNodeJS=`node --version` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagCommit=`git rev-parse HEAD` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagYarn=`yarn --version` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagCompilationTime=`TZ=UTC date +%Y-%m-%dT%H:%M:%S+0000`" ./cmd/golang-url-shortener
...
buildDockerImage:
	...
	CGO_ENABLED=0 gox -output="docker_releases/{{.Dir}}_{{.OS}}_{{.Arch}}/{{.Dir}}" -osarch="linux/amd64 linux/arm" -ldflags="-X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagNodeJS=`node --version` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagCommit=`git rev-parse HEAD` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagYarn=`yarn --version` -X github.com/russelltsherman/golang-url-shortener/internal/util.ldFlagCompilationTime=`TZ=UTC date +%Y-%m-%dT%H:%M:%S+0000`" ./cmd/golang-url-shortener
